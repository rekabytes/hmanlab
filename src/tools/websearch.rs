...
async fn search_mcp_parallel(
    ...
    let text = call_mcp("https://search.parallel.ai/mcp", payload, auth_header.as_deref()).await?;
    Ok(truncate_output(&text, MAX_OUTPUT_CHARS))
}
...
pub async fn tool_web_fetch(args: &Value, _ctx: &ToolContext) -> Result<String> {
    ...
    if bytes.len() > MAX_FETCH_BYTES {
        bail!("web_fetch: response too large ({} bytes, limit 5 MB)", bytes.len());
    }
...
fn format_results(
    ...
    for (i, r) in results.enumerate() {
        ...
        out.push_str(&format!(
            "{}. {}\n   {}\n   {}\n\n",
            i + 1,
            if r.title.is_empty() { r.url.clone() } else { r.title },
            r.url,
