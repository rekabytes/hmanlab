...
pub(super) fn tool_arg_summary(name: &str, args: &serde_json::Value) -> String {
    ...
    match name {
        ...
        "list_dir" | "edit_file" | "multi_edit" | "apply_patch" | "move_lines"
        | "delete_lines" | "insert_at" | "write_file" => s("path")
            .map(str::to_string)
        ...
        "save_memory" | "read_memory" | "forget_memory" => {
            s("key").map(str::to_string).unwrap_or_else(|| fallback_json(args))
        }
...
mod tests {
    use super::{
        ...
        pulse_modifier, progress_bar, split_thinking, summarize_group, tool_arg_summary,
        tool_icon, tool_label, ToolGroup,
    };
    use crate::ollama::ChatMessage;
    ...
    fn unknown_tool_falls_back_to_json() {
        ...
        assert_eq!(tool_arg_summary("future_tool", &v), r#"{"something":"else"}"#);
    }
    ...
    fn groups_maximal_runs_of_tool_messages() {
        let msgs = vec![
            ChatMessage { role: "assistant".into(), ..Default::default() },
    ...
    fn permission_card_breaks_a_run() {
        ...
        let msgs = vec![
            ...
            ChatMessage { role: "permission".into(), ..Default::default() },
