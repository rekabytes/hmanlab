...
use super::helpers::{
    ...
    pulse_modifier, progress_bar, split_thinking, summarize_group, tool_arg_summary, tool_label,
...
pub(in crate::ui) fn render_chat(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    let group_by_start: std::collections::HashMap<usize, ToolGroup> = groups
        .iter()
        .map(|g| (g.start, *g))
        .collect();
    ...
    for (i, msg) in app.messages.iter().enumerate() {
        ...
        if in_multi_group {
            ...
            match (is_group_start, group_expanded) {
                ...
                (true, false) => {
                    let tile = render_grouped_tile(
                        &app.messages, group, app, inner.width,
                    );
                ...
                (true, true) => {
                    let header = render_group_header(
                        &app.messages, group, app, inner.width,
                    );
        ...
        let render_tile_only = is_tool
            && !app.expanded_tools.contains(&i)
            && !in_expanded_group;
...
fn render_collapsed_tile(msg: &ChatMessage, idx: usize, app: &App, inner_w: u16) -> CollapsedTile {
    ...
    let spent: usize = spans
        ...
        .sum();
    let arg_budget = (inner_w as usize)
        .saturating_sub(spent + 2 + 3)
        .max(8);
    let arg_text = if arg_text.chars().count() > arg_budget {
        let cut: String = arg_text.chars().take(arg_budget.saturating_sub(1)).collect();
        format!("{cut}…")
    } else {
        arg_text
    };
...
fn render_grouped_tile(
    ...
    let batch_pending = progress.in_flight
        || (group.count() < progress.total && app.turn.is_generating());
    ...
    if let Some(joined) = arg_text {
        ...
        let budget = (inner_w as usize)
            .saturating_sub(spent + 3 + 6 + 1)
            .max(8);
...
fn render_permission_card(
    ...
    out.push(Line::from(vec![
        ...
        Span::styled(top_title, Style::default().fg(accent).add_modifier(Modifier::BOLD)),
    ...
    for paragraph in msg.content.split('\n') {
        ...
        for chunk in crate::ui::markdown::wrap_styled_segments(segments, inner_w) {
            ...
            let mut spans = vec![Span::styled(prompt_pad_l, Style::default().fg(accent).bg(bg))];
            for s in chunk {
            ...
            spans.push(Span::styled(prompt_pad_r, Style::default().fg(accent).bg(bg)));
    ...
    out.push(Line::from(vec![
        ...
        Span::styled(
            hint,
            Style::default().fg(c::FG_DIM).bg(bg).add_modifier(Modifier::ITALIC),
        ),
        Span::styled(pad, Style::default().bg(bg)),
        Span::styled(prompt_pad_r, Style::default().fg(accent).bg(bg)),
    ]));
    ...
    out.push(Line::from(Span::styled(bottom, Style::default().fg(accent))));
