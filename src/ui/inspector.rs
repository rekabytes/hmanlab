...
impl Section for ActiveToolSection {
    ...
    fn draw(&self, f: &mut Frame, area: Rect) {
        let line = Line::from(vec![
            ...
            Span::styled(format!("  · {}", self.elapsed), Style::default().fg(color::FG_DIM)),
        ]);
...
impl ShellSection {
    fn from_app(app: &App) -> Option<Self> {
        ...
        let status: Line<'static> = if rt.running {
            Line::from(Span::styled("running", Style::default().fg(color::STATE_TOOLING)))
        } else {
            match rt.exit_code {
                Some(Some(0)) => Line::from(Span::styled("done", Style::default().fg(color::SUCCESS))),
                Some(Some(c)) => Line::from(Span::styled(
                    format!("exit {c}"),
                    Style::default().fg(color::ERROR),
                )),
                Some(None) => Line::from(Span::styled("killed", Style::default().fg(color::WARNING))),
...
impl Section for ShellSection {
    ...
    fn draw(&self, f: &mut Frame, area: Rect) {
        ...
        spans.push(Span::styled(format!(" · {}", self.elapsed_s), Style::default().fg(color::FG_DIM)));
