    ...
    let items: Vec<ListItem> = MCP_PROVIDERS
        ...
        .map(|(i, p)| {
            ...
            let badge = if is_active {
                ...
                if !p.needs_key { " [no key needed]" } else { " [configured]" }
            } else {
                " [not configured]"
            };
...
fn render_key_input(f: &mut Frame, area: Rect, app: &App) {
    ...
    let body = Paragraph::new(vec![
        Line::from(vec![
            ...
            Span::styled(provider.signup_url, Style::default().fg(theme::color::ACCENT)),
        ]),
        Line::from(vec![
            Span::styled(provider.free_tier, Style::default().fg(theme::color::FG_DIM)),
        ]),
    ])
