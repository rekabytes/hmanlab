...
pub(crate) fn initial_expanded(workspace: &Path, show_hidden: bool) -> std::collections::HashSet<std::path::PathBuf> {
    ...
    const SKIP_DIRS: &[&str] = &[
        ".git", ".svn", ".hg", ".jj", "target", "node_modules", "dist",
        "build", "out", "__pycache__", ".next", ".nuxt", ".turbo",
...
pub(super) fn render_sidebar(f: &mut Frame, area: Rect, app: &mut App) {
    ...
    for (i, (label, kind)) in tabs.iter().enumerate() {
        ...
        let w = if i as u16 == n - 1 {
            tab_strip.width.saturating_sub((i as u16).saturating_mul(cell_w))
        } else {
            cell_w
        };
    ...
    if app.local_session_summaries.is_empty() {
        ...
        if app.local_session_summaries.len() > 40 {
            lines.push(Line::from(Span::styled(
                format!(
                    "  …(+{} more)",
                    app.local_session_summaries.len() - 40
                ),
                Style::default().fg(theme::color::FG_DIMMER),
            )));
        }
